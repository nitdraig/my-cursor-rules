...
- `.cursorrules` and/or `AGENTS.md` — stack rules, folder layout, English-only comments.
- Match package manager from recon (`pnpm`, `npm`, `yarn`).
...
(Exact commands from recon — not guessed. Prefer `rg` / `git grep` over shell-specific `grep` for pattern checks.)

**Cross-platform pattern check** (pick one that works in the executor environment):

- `rg -n "<old pattern>" src/`
- `git grep -n "<old pattern>" -- src/`
...
- [ ] `rg -n "<old pattern>" src/` (or `git grep`) returns no matches
- [ ] No files outside the in-scope list are modified (`git status`)
- [ ] `plans/README.md` status row updated (unless reviewer-maintained index)

## Rollback (P1 / HIGH risk only)

Include when Risk is HIGH or the change is hard to revert:

- How to undo (revert commit, feature flag off, restore migration down).
- Smoke check after rollback: `<command>` → expected result.
