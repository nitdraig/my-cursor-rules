...
- Frontend (if applicable): bundle composition (heavyweight deps for trivial use), missing code-splitting on rarely-hit routes, unoptimized images/fonts, client-side fetching for data available at render time, render waterfalls. For React/Next.js, use `vercel-react-best-practices` when installed.
- **Accessibility** (if UI): missing labels, keyboard traps, contrast — use `web-design-guidelines` for review depth.
- Backend: synchronous work that belongs in a queue, missing indexes implied by query patterns (flag for verification — don't claim without schema evidence), connection-per-request patterns where pooling exists. Express APIs: `express-api-review` performance section.
...
- **React Query / data layer** (when applicable): `useQuery` / `useMutation` inline in components instead of feature `hooks/`; ad-hoc query key strings instead of `api/query-keys`; hardcoded API URLs instead of `api/endpoints`.
- **Express** (when applicable): business logic in route files; fat controllers; missing global error handler; validation not in reusable middleware. Deep review: load `express-api-review` skill.
- **Comment / log hygiene**: debug `console.log`, obsolete TODOs, large commented-out blocks, non-English comments where project requires English (see `cleaning-code` skill).
...
- Missing agent guidance: no `AGENTS.md`, `.cursorrules`, or OpenCode-friendly `opencode.json` / skills path — high leverage for plan executors; recommend outline as a plan.
- **i18n / language drift**: docs, comments, or error messages not in the project's required language (often English per `.cursorrules`).
